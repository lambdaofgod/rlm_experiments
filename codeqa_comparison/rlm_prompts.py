...
def _sub_llm_char_limit(max_tokens: int) -> str:
    """Return a human-readable char limit for sub-LLM prompts based on context size."""
...
2. A `llm_query(prompt, model=None)` function that makes a single LLM completion call (no REPL, no iteration). Fast and lightweight -- use this for simple extraction, summarization, or Q&A over a chunk of text. The sub-LLM can handle around __CHAR_LIMIT__ chars.
